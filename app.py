...
def regular_admin():
    if request.method == 'POST':
        ...
        file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        file.save(file_path)
        shutil.move('../BunsValidation/uploads/' + filename,
                    '../BunsValidation/static/imageTesting/' + commentaire + '@' + Type + '@' + filename)
...
def checkimages():
    image_names = os.listdir('../BunsValidation/static/imageTesting/')  # List of images to be validated
...
def Delete(name):
    os.remove("../BunsValidation/static/imageTesting/" + name)
